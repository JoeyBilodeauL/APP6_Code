    ...
    while (1) {
        if (inputBufferFull) {
            ...
            if (SWT_GetValue_Local(0) == 0) { // MAIN DECISION TREE: PASS-THROUGH
				...
				//          Though this buffer is currently being filled by the A/D interrupt handler,
				//          the last block is still available for a short time.
                //       2) All three blocks from the most recently filled input buffer pointed
				//          to by "previousInBuffer".
